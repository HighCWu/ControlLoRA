...
from control_lora.datasets.annotator.uniformer import UniformerDetector


class UniformerDataset(BaseDataset):
    def __init__(self, zfill=16, **kwargs):
        super().__init__(**kwargs)

        self.zfill = zfill

        guide_dir = os.path.join(self.root_dir, self.base_name + '-uniformer')
        if not os.path.exists(guide_dir) or len(glob.glob(guide_dir + '/*.png')) != len(self.data):
            ...
            print('Generate dataset of Uniformer segment guide.')
            for i in tqdm(range(len(self.data))):
                ...
                detected_map = apply_uniformer(resize_image(image, self.resolution))
